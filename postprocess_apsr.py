...
class PostProcessAPSR:
    ...
    def LoadCredentials(self) -> Dict[str, str]:
        if os.path.exists(self.credentials_path):
            ...
            try:
                ...
                fn, line = self.utils.GetFuncLine()
                self.utils.Log("error", fn, line, f"Failed to load credentials: {e}")
                return None
            return credentials
    ...
    def WebFetch(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if response.status_code == 429:
                    ...
                    fn, line = self.utils.GetFuncLine()
                    self.utils.Log("warning", fn, line, msg)
                    print(msg)
                ...
                fn, line = self.utils.GetFuncLine()
                self.utils.Log(
                    "error", fn, line, f"Webfetch failed for {url}; Exception: {e}"
                )
        ...
        fn, line = self.utils.GetFuncLine()
        self.utils.Log(
            "error", fn, line, f"All {max_attempts} attempts failed for {url}."
        )
        return None
    ...
    def LoadCamCoreCSV(self) -> None:
        try:
            ...
            fn, line = self.utils.GetFuncLine()
            self.utils.Log("error", fn, line, f"Failed to load Cambridge Core CSV: {e}")
    ...
    def PostProcessCambridgeCoreCitations(self) -> None:
        ...
        try:
            if self.camb_core_df is None:
                fn, line = self.utils.GetFuncLine()
                self.utils.Log(
                    "error", fn, line, "No input data loaded. Cannot process citations."
                )
                return
            ...
            if self.loaded_cc_apsr_citations_cache:
                ...
                if len(self.cc_apsr_citations_cache) == pre_update_len:
                    fn, line = self.utils.GetFuncLine()
                    self.utils.Log("info", fn, line, "No new citation data to process.")
                    return
            else:
                with self.cc_apsr_citation_lock:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        fn, line = self.utils.GetFuncLine()
                        self.utils.Log(
                            "error", fn, line, f"Failed to process citation row: {e}"
                        )
            ...
            fn, line = self.utils.GetFuncLine()
            self.utils.Log(
                "error", fn, line, f"Failed to process Cambridge Core citations: {e}"
            )
            raise e
    ...
    def PostProcessCambridgeCoreDOIs(self) -> None:
        ...
        try:
            if self.camb_core_df is None:
                fn, line = self.utils.GetFuncLine()
                self.utils.Log(
                    "error", fn, line, "No input data loaded. Cannot process DOIs."
                )
                return
            ...
            if self.loaded_cc_apsr_dois_cache:
                ...
                if len(self.cc_apsr_dois_cache) == pre_update_len:
                    fn, line = self.utils.GetFuncLine()
                    self.utils.Log("info", fn, line, "No new APSR DOI data to process.")
                    return
            else:
                with self.cc_apsr_dois_lock:
            ...
            fn, line = self.utils.GetFuncLine()
            self.utils.Log(
                "error", fn, line, f"Failed to process Cambridge Core DOIs: {e}"
            )
            raise e
    ...
    def PostProcessCrossRef(self, min_abstract_char_length: int = 25) -> None:
        ...
        try:
            ...
            if len(to_process) == 0:
                fn, line = self.utils.GetFuncLine()
                self.utils.Log("info", fn, line, "No new abstracts to process.")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        fn, line = self.utils.GetFuncLine()
                        self.utils.Log(
                            "error", fn, line, f"Failed to process abstract row: {e}"
                        )
            ...
            fn, line = self.utils.GetFuncLine()
            self.utils.Log("error", fn, line, f"Failed to process abstracts: {e}")
            raise e
    ...
    def FinalOutput(self, min_abstract_char_length: int = 25) -> None:
        ...
        try:
            ...
            fn, line = self.utils.GetFuncLine()
            self.utils.Log("error", fn, line, f"Failed to produce final output: {e}")
            raise e
...
if __name__ == "__main__":
    ...
    try:
        ...
        if pproc:
            ...
            pproc.utils.Log("info", fn, line, "Post-processing interrupted by user.")
        ...
        fn, line = pproc.utils.GetFuncLine()
        pproc.utils.Log("error", fn, line, f"Post-processing failed: {e}")
        raise e
